...
int hashExample()
{
    ...
    size_t max_count =1024;                                      // Maximum number of elements to insert
    ...
    while(mlf <= 1.5f)                                            // Loop until max load factor is 1.5
    {
        ...
        for(size_t n_elements =0; n_elements < max_count; ++n_elements)
        {
            lf = people.load_factor();                               // Record load factor
            size_t val= value+1; 
            people.emplace(make_pair(string("name"+(++value)), value));
